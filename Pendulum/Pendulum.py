...
def create_model(input_shape, action_space):
    ...
    #Output layer 1 unit for torque
    X = Dense(units=action_space, activation='linear', kernel_initializer='he_uniform')(X)
    
    model = Model(inputs=X_input, outputs=X)
    model.compile(loss="mse", optimizer=RMSprop(learning_rate=0.001, rho=0.95, epsilon=0.01), metrics=["accuracy"])
    
    return model

class DQNAgent:
    """
    Create DQNAgent that will train on Pendulum data
    """
    
    def __init__(self):
        self.env = gym.make("Pendulum-v1")
        self.state_size = self.env.observation_space.shape[0]
        self.action_space = 1 # refer to https://github.com/openai/gym/blob/master/gym/envs/classic_control/pendulum.py
        self.EPISODES = 1000
    ...
    def run(self):
        for eps in range(self.EPISODES):
            ...
            while not done:
                ...
                i += 1
                if done: 
                    print("episode: {}/{}, epsilon: {}".format(eps, self.EPISODES, self.epsilon))
